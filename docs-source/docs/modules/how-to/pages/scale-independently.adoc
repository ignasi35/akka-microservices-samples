...
Not all nodes in an Akka Cluster need to be identical. When starting each process you can add some metadata to it to give that instance a special purpose. {akka}/typed/cluster.html#node-roles[Node Roles] in Akka Cluster will handle that metadata.
...
* serves gRPC traffic,
* hosts `Cart Entity` 's (that is, {akka}/typed/persistence.html[`EventSourcedBehavior` 's])
* runs three different {akka-projection}[projections].
...
You can {akka}/typed/cluster-sharding.html[specify a role when declaring the entity]:
...
* {akka}/typed/cluster.html#node-roles[Akka Cluster Node Roles]
*  {akka}/typed/cluster-sharding.html[Akka Cluster Sharding]
* {akka}/typed/cluster-sharded-daemon-process.html#sharded-daemon-process[Sharded Daemon Process]
