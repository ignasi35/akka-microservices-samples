...
This guide assumes you created a new project using the {akka-microservices-seed-scala}[Akka Microservicies template] with the following command:
...
The project produced with the {akka-microservices-seed-scala}[Akka Microservices template] assumes the application will use the {akka-persistence-cassandra}[Akka Persistence Cassandra plugin]. You can opt out from using that plugin and use the {akka-persistence-jdbc}[Akka Persistence JDBC] or other plugins instead.
...
The project produced with the {akka-microservices-seed-scala}[Akka Microservices template] assumes the application will use a Cassandra database to store the {akka-projection}/overview.html#overview[offset tracking]  required by the projections. You can replace the artifact to use an RDBMS:
...
Replacing the artifact is only the first step. Each of the implementations of Offset Stores for  Akka Projections has slightly different APIs. Refer to the {akka-projection}[Akka Projections reference documentation] for more details.

== Remove support for Kafka

The project produced with the {akka-microservices-seed-scala}[Akka Microservices template] projects data include the necessary dependencies to project data to a Kafka topic. If you are not projecting data into Kafka, remove the version declaration and the artifact:
...
If you are not projecting your data to a read-side you can remove all the artifacts provided by {akka-projection}[Akka Projections]:
