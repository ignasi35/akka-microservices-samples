...
One way to meet the two requirements above is to ensure only one copy of the data is on memory at any given time. That's also known as {reactive-principles}/patterns/isolate-mutations.html[isolating the mutations] {reactive-principles}/patterns/isolate-mutations.html[{tab-icon}, window="tab"]. Then, because your data may exceed the capacity of your memory, you can shard the data and keep a chunk on each of your application nodes. Finally, each shard of data uses mutual exclusion mechanisms to ensure each mutation happens sequentially.
