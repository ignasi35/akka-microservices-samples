...
* xref:properties/isolation.adoc[], where each microservice is completely decoupled so that its lifecycle, including failure, has no impact on other microservices in the system.
* xref:properties/autonomy.adoc[], where each microservice acts and makes decisions independently and publishes its behavior through an API.
...
* xref:properties/mobility.adoc[], where a microservice can be moved at runtime but can be reached in the same way regardless of its location.
...
Messaging does not need to be point-to-point. Use of messaging can, and often does, mean adopting an xref:properties/message-driven-event-driven.adoc[event-driven] architecture, which can bring additional benefits. Event-driven systems promote autonomy and decoupling, allowing the development organization and the resulting system to scale more easily. They provide good options for managing consistency and persistence. 
...
If the message is important, you need some way of persisting it to make sure it will be dealt with at some point in time. An xref:properties/message-driven-event-driven.adoc[event-driven] architecture offers several ways of handling this. For example, in a microservices system, you could use a message broker with delivery guarantees, or write such messages to a database or log. If a reply is required, the sender could just wait for an acknowledgement that the request was received and continue its work, expecting the answer to the question later.
...
An event log provides reliable auditing and simplifies debugging. When the log is provided by a messaging service, other microservices and legacy applications can subscribe to events of interest. And, in the case of failure, it is possible to replay the log at any time. This pattern is referred to as xref:techniques/event-sourcing.adoc[Event Sourcing].
