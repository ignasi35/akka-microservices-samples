...
Microservices should be xref:properties/isolation.adoc[isolated] and xref:properties/autonomy.adoc[autonomous].
...
* Databases and other persistent storage can be seen as another way that nodes of a service communicate. For microservices that use persistent entities, we encourage xref:techniques/event-sourcing.adoc[Event Sourcing] , which also takes advantage of asynchronous communication and provides guarantees via an event log.
