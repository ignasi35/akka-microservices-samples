...
In this part, we will create an {akka-projection}/[Akka Projection] from the events of the `ShoppingCart` entity. The Projection updates counts in the database to keep track of how popular the items are. The database can be queried for how popular an item is, which is a query that the `ShoppingCart` entities themselves can't answer since they can only be addressed by individual cart identifiers.
...
Here we use five different tags. The tagging is not easy to change later without system downtime. Before going live in production you should consider how many tags to use, see {akka-projection}/running.html[Akka Projections reference documentation] for more information.
...
* {akka-projection}/[Akka Projection reference documentation].
* {akka}/typed/cluster-sharded-daemon-process.html[Akka Sharded Daemon Process reference documentation].
